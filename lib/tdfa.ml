...
module Make (Tag : T) = struct 
  ...
  let add_tag state tag tagger = 
    StateMap.add state tag tagger 
  ...
  let determinise n =
    ...
    let gen_state =
      ...
      fun () ->
        ...
        s
    in
    let rec build nfa_state (mapping, states, transitions, finals, rejecting, tagger) =
      match M.find nfa_state mapping with
      | dfa_state -> (dfa_state, mapping, states, transitions, finals, rejecting, tagger)
      | exception Not_found ->
          let dfa_state = gen_state () in
          let mapping = M.add nfa_state dfa_state mapping in
          let finals =
            if TaggedNfa.is_accepting n nfa_state then StateSet.add dfa_state finals
            else finals
          in
          let rejecting = 
            if TaggedNfa.is_rejecting n nfa_state then Some(dfa_state) 
            else rejecting in 
          let tagger = add_tag dfa_state (TaggedNfa.emit_tag n nfa_state) tagger in 
          ...
          let builder c (m, s, t, f, r, tg) =
            ...
            let dfa_next_state, m', s', t', f', r', tg' = build next_state (m, s, t, f, r, tg) in
    ...
    let initial, _, states, transitions, finals, rejecting, tagger =
      build nfa_initial (M.empty, StateSet.empty, StateMap.empty, StateSet.empty, None, StateMap.empty)
    in
    let next s =
    ...
    match rejecting with
      ...
      | Some(rejecting) -> { states; initial; finals; next; alphabet; rejecting; tagger }
  ...
  let step t_dfa q c = 
    try CharMap.find c (t_dfa.next q) with
      | Not_found -> t_dfa.rejecting
